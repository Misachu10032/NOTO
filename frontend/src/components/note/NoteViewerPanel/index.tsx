...
export default function NoteViewerPanel() {
  ...
  const tempNote = {
    ...rawTempNote,
    followupQuestions: Array.isArray((rawTempNote as any).followupQuestions) ? (rawTempNote as any).followupQuestions : [],
    followupAnswers: Array.isArray((rawTempNote as any).followupAnswers) ? (rawTempNote as any).followupAnswers : [],
  };
  ...
  const handleAskFollowUp = async () => {
    ...
    setTimeout(() => {
      ...
      addTempNoteFollowUpQuestion({ id: tempNote.id, followupQuestion: followUp });
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div className="bg-white shadow-md rounded-lg p-4 flex flex-col">
        ...
        <div className="mt-6">
          <label className="block font-medium mb-2 text-gray-700">Ask a follow-up question:</label>
          <div className="flex gap-2">
            <input
          ...
          {isAsking && (
            <p className="text-sm text-gray-400 mt-2 animate-pulse">Thinking...</p>
          )}
          {followUpAnswer && (
            <div className="mt-3 p-3 bg-green-50 border border-green-200 rounded-md text-sm">
              <strong className="text-green-700">Answer:</strong> {followUpAnswer}
            </div>
          )}
          ...
          {tempNote.followupQuestions.length > 0 && (
            <div className="mt-6">
              <h3 className="font-semibold mb-2 text-gray-700">Previous Follow-ups:</h3>
                ...
                {tempNote.followupQuestions.map((q: string, idx: number) => (
                  ...
                  >
                    ...
                    {(tempNote.followupAnswers || [])[idx] && (
                      <p>
                        <span className="font-medium text-gray-800">A:</span> {(tempNote.followupAnswers || [])[idx]}
                      </p>
                    )}
                  </li>
                ))}
